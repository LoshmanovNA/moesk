...
class RegistrationPage(BasePage):
    ...
    _locators = RegistrationPageLocators()
    ...
    def select_user_type_fl(self):
        ...
        self.click(self._locators.REGISTRATION_USER_TYPE_LIST_CSS)
        # Выбираем тип пользователя ФЛ
        self.click(self._locators.REGISTRATION_USER_TYPE_FL_XPATH, 'By.XPATH')
    ...
    def select_user_type_ul(self):
        ...
        self.click(self._locators.REGISTRATION_USER_TYPE_UL_XPATH, 'By.XPATH')
    ...
    def select_user_type_ip(self):
        ...
        self.click(self._locators.REGISTRATION_USER_TYPE_IP_XPATH, 'By.XPATH')
    ...
    def select_user_type_representative(self):
        ...
        self.click(self._locators.REGISTRATION_USER_TYPE_PPD_XPATH, 'By.XPATH')
    ...
    def fill_registration_form(self, user_model):
        ...
        try:
            ...
            if self.is_text_visible('Физическое лицо') or self.is_text_visible('Представитель по доверенности'):
                self.update_text(self._locators.REGISTRATION_FIRST_NAME_CSS, user_model.first_name)
                self.update_text(self._locators.REGISTRATION_LAST_NAME_CSS, user_model.last_name)
                self.update_text(self._locators.REGISTRATION_PATRONYMIC_CSS, user_model.patronymic_name)
                
            # Если тип пользователя Юридическое лицо
            elif self.is_text_visible('Юридическое лицо'):
                self.update_text(self._locators.REGISTRATION_UL_FULL_NAME_CSS, user_model.company_full_name)
                self.update_text(self._locators.REGISTRATION_UL_SHORT_NAME_CSS, user_model.company_short_name)
                self.update_text(self._locators.REGISTRATION_CONTACT_FIRST_NAME_CSS, user_model.first_name)
                self.update_text(self._locators.REGISTRATION_CONTACT_LAST_NAME_CSS, user_model.last_name)
                self.update_text(self._locators.REGISTRATION_CONTACT_PATRONYMIC_NAME_CSS, user_model.patronymic_name)

            # Если тип пользователя ИП
            elif self.is_text_visible('Индивидуальный предприниматель'):
                self.update_text(self._locators.REGISTRATION_IP_FIRST_NAME_CSS, user_model.first_name)
                self.update_text(self._locators.REGISTRATION_IP_LAST_NAME_CSS, user_model.last_name)
                self.update_text(self._locators.REGISTRATION_IP_PATRONYMIC_NAME_CSS, user_model.patronymic_name)
                self.update_text(self._locators.REGISTRATION_CONTACT_FIRST_NAME_CSS, user_model.first_name)
                self.update_text(self._locators.REGISTRATION_CONTACT_LAST_NAME_CSS, user_model.last_name)
                self.update_text(self._locators.REGISTRATION_CONTACT_PATRONYMIC_NAME_CSS, user_model.patronymic_name)
        except:
        ...
        self.update_text(self._locators.REGISTRATION_PHONE_CSS, user_model.phone)
        ...
        if self.env == 'production':
            ...
            self.update_text(self._locators.REGISTRATION_EMAIL_CSS, self.temp_mail.email())
            self.logger.info(self.temp_mail.email())
        else:
            # Если на тесте, то генерируем фейковый валидный email
            self.update_text(self._locators.REGISTRATION_EMAIL_CSS, user_model.email)

    @allure.step
    def actions_with_required_checkboxes(self, check_box_1=True, check_box_2=True):
        # Ставим чек-боксы
        if check_box_1:
            self.js_click(self._locators.REGISTRATION_CONFIRM1_CSS)
        if check_box_2:
            self.js_click(self._locators.REGISTRATION_CONFIRM2_CSS)

    @allure.step
    def continue_registration(self):
        # Кликаем кнопку продолжения регистрации
        self.click(self._locators.REGISTRATION_NEXT_STEP_CSS)
    ...
    def get_actual_validation_errors(self, error_model):
        ...
        registration_form_elements = self._locators.REGISTRATION_VALIDATION_ERROR_MESSAGES_XPATH.items()
